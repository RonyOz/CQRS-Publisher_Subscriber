...
/**
 * Subscriber A - Receives messages from Azure Service Bus Topic Subscription
 */
async function main() {
  ...
  const subscriptionName = 'subscription-a';
...
╚════════════════════════════════════════════╝
    ...
    const messageHandler = async (message: any) => {
      try {
        ...
        console.log(`📨 Message received by Subscriber A:`);
    ...
    // Keep running
    console.log('✅ Subscriber A is active and listening...');
    console.log('Press Ctrl+C to exit.\n');

    // Handle graceful shutdown
    process.on('SIGINT', async () => {
      console.log('\n\n🛑 Shutting down Subscriber A...');
